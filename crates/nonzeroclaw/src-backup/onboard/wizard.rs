...
use crate::onboard::migration::{
    self, build_config_migration_plan, detect_openclaw_installation, plan_present_fields,
    ChannelAssignment, ChannelOwner, MemoryMigrationOptions, MemoryMigrationOutcome,
...
pub async fn run_wizard(force: bool) -> Result<Config> {
    ...
    print_step(2, 10, "OpenClaw Migration (optional)");
    let migration_result = setup_openclaw_migration(&workspace_dir).await;

    print_step(3, 10, "AI Provider & API Key");
    let (provider, api_key, model, provider_api_url) = setup_provider(&workspace_dir).await?;

    print_step(4, 10, "Channels (How You Talk to ZeroClaw)");
    let channels_config = setup_channels_with_migration(setup_channels()?, &migration_result)?;
...
// ── Step 2: OpenClaw Migration ───────────────────────────────────

/// Outcome of the migration wizard step, threaded through later steps.
#[derive(Debug, Default)]
pub(crate) struct MigrationWizardResult {
    /// The detected OpenClaw installation, if any.
    pub install: Option<OpenClawInstallation>,
    /// Channel assignment decisions made by the user.
    pub channel_assignments: Vec<ChannelAssignment>,
    /// Whether the user confirmed the config field migration.
    pub config_migration_confirmed: bool,
    /// Outcome of the memory migration step.
    pub memory_outcome: Option<MemoryMigrationOutcome>,
}

/// Run the OpenClaw migration wizard step.
///
/// Always succeeds — if no OpenClaw installation is found, or the user skips,
/// returns a default [`MigrationWizardResult`] and continues normally.
async fn setup_openclaw_migration(workspace_dir: &Path) -> MigrationWizardResult {
    // Detection — silent, not a hard error if detection fails.
    let install = match detect_openclaw_installation() {
        Ok(Some(i)) => i,
        Ok(None) => {
            println!(
                "  {} No existing OpenClaw installation found — skipping migration.",
                style("○").dim()
            );
            return MigrationWizardResult::default();
        }
        Err(e) => {
            println!(
                "  {} OpenClaw detection error (skipping migration): {}",
                style("!").yellow(),
                e
            );
            return MigrationWizardResult::default();
    ...
    println!(
        "  {} OpenClaw installation found: {}",
        style("✓").green().bold(),
        style(migration::installation_summary(&install)).green()
    );
    println!(
        "  {} Config: {}",
        style("›").cyan(),
        style(install.config_path.display()).dim()
    );
    if let Some(ref ws) = install.workspace_path {
        println!(
            "  {} Workspace: {}",
            style("›").cyan(),
            style(ws.display()).dim()
        );
    }
    println!();

    let migrate = match Confirm::new()
        .with_prompt("  Migrate settings from OpenClaw to NZC?")
        .default(true)
        .interact()
    {
        Ok(v) => v,
    ...
    if !migrate {
        println!("  {} Skipping OpenClaw migration.", style("○").dim());
        return MigrationWizardResult {
            install: Some(install),
            ..Default::default()
        };
    ...
    // ── Channel assignment ──
    let channel_assignments = run_channel_assignment_step(&install);

    // ── Config field mapping ──
    let config_migration_confirmed = run_config_migration_step(&install);

    // ── Memory migration ──
    let memory_outcome = run_memory_migration_step(&install, workspace_dir).await;

    MigrationWizardResult {
        install: Some(install),
        channel_assignments,
        config_migration_confirmed,
        memory_outcome: Some(memory_outcome),
    }
}

/// Interactively assign each detected channel to NZC, OpenClaw, or Skip.
fn run_channel_assignment_step(install: &OpenClawInstallation) -> Vec<ChannelAssignment> {
    if install.channels.is_empty() {
        println!(
            "  {} No channels configured in OpenClaw — nothing to assign.",
            style("○").dim()
        );
        return Vec::new();
    }
    ...
    println!(
        "  {}",
        style("Channel Assignment").white().bold()
    );
    println!(
        "  {}",
        style("Each channel can have exactly one owner. Decide who handles each one.").dim()
    );
    println!();

    let owner_labels = ["NZC (this install)", "OpenClaw (leave unchanged)", "Skip"];
    let owners = [ChannelOwner::Nzc, ChannelOwner::OpenClaw, ChannelOwner::Unassigned];

    let mut assignments = Vec::new();

    for channel in &install.channels {
        let cred_note = if channel.has_credentials {
            "has credentials"
        } else {
            "no credentials found"
        };
        let enabled_note = if channel.enabled { "enabled" } else { "disabled" };

        let prompt = format!(
            "  Who owns {} ({enabled_note}, {cred_note})?",
            style(&channel.name).cyan().bold()
        );
        ...
        let owner = match choice {
            Ok(idx) => owners[idx].clone(),
            Err(_) => ChannelOwner::Unassigned,
        };

        println!(
            "  {} {} → {}",
            style("✓").green(),
            style(&channel.name).cyan(),
            style(owner.to_string()).green()
        );
    ...
    // Summary
    let nzc_count = assignments.iter().filter(|a| a.owner == ChannelOwner::Nzc).count();
    let oc_count = assignments.iter().filter(|a| a.owner == ChannelOwner::OpenClaw).count();
    let skip_count = assignments.iter().filter(|a| a.owner == ChannelOwner::Unassigned).count();
    println!();
    println!(
        "  {} Channel assignment: {} → NZC, {} → OpenClaw, {} skipped",
        style("✓").green().bold(),
        nzc_count,
        oc_count,
        skip_count
...
/// Show the config migration diff and get user confirmation.
///
/// Returns `true` if the user confirmed, `false` if skipped.
fn run_config_migration_step(install: &OpenClawInstallation) -> bool {
    let plan = build_config_migration_plan(&install.config);
    let present = plan_present_fields(&plan);

    println!();
    println!("  {}", style("Config Migration").white().bold());

    if present.is_empty() {
        println!(
            "  {} No directly mappable config fields found.",
            style("○").dim()
        );
        return false;
    }
    ...
    println!(
        "  {} The following settings can be copied to NZC config:",
        style("›").cyan()
    );
    println!();

    for field in &present {
        // Redact credential values in display.
        let display_value = match &field.value {
            Some(serde_json::Value::String(s)) if looks_like_credential(&field.openclaw_path) => {
                format!("[{} chars, redacted]", s.len())
            }
            Some(v) => v.to_string(),
        ...
        println!(
            "    {} {} → {}  =  {}",
            style("•").dim(),
            style(&field.openclaw_path).dim(),
            style(&field.nzc_path).cyan(),
            style(&display_value).green()
        );
    }
    ...
    if !plan.unmapped.is_empty() {
        println!();
        println!(
            "  {} The following OpenClaw settings have no NZC equivalent (will be skipped):",
            style("!").yellow()
        );
    ...
    println!();
    println!(
        "  {} OpenClaw config is read-only — NZC will NOT modify {}",
        style("🔒").cyan(),
        style(install.config_path.display()).dim()
    );

    match Confirm::new()
        .with_prompt("  Apply these settings to NZC config?")
        .default(true)
        .interact()
    {
        Ok(v) => v,
...
/// Offer LLM-assisted memory migration. Always returns, never hard-fails.
async fn run_memory_migration_step(
    ...
    if !install.has_memory() {
        println!();
        println!(
            "  {} No OpenClaw memory files found — skipping memory migration.",
            style("○").dim()
        );
        return MemoryMigrationOutcome::NoMemoryFound;
    }

    let file_count = install.memory_files().len();
    println!();
    println!("  {}", style("Memory Migration (optional)").white().bold());
    println!(
        "  {} Found {} memory file(s) in OpenClaw workspace.",
        style("›").cyan(),
        file_count
    );
    println!(
        "  {} {}",
        style("ℹ").cyan(),
        style("OpenClaw memory contains operational details specific to OpenClaw.").dim()
    );
    println!(
        "  {} {}",
        style("ℹ").cyan(),
        style("Migration uses your configured LLM to clean and reframe content for NZC.").dim()
    );
    println!();

    let do_migrate = match Confirm::new()
        .with_prompt(
            "  Migrate OpenClaw memory to NZC now? (You can also run `nzc migrate-memory` later)",
        )
        .default(false)
        .interact()
    {
    ...
    if !do_migrate {
        let source_files = install.memory_files();
        println!(
            "  {} Memory migration skipped. Run `nzc migrate-memory` later.",
            style("○").dim()
        );
        if let Some(ref ws) = install.workspace_path {
            println!(
        ...
        return MemoryMigrationOutcome::Skipped { source_files };
    }

    println!(
        "  {} Running LLM-assisted memory migration…",
        style("⟳").cyan()
    );

    let opts = MemoryMigrationOptions {
        dest_workspace: workspace_dir.to_path_buf(),
        max_content_bytes: 64 * 1024,
    };

    // TODO (session 3/4): replace StubLlmFn with NZC's real provider client once
    // the provider infrastructure is wired up here.  The stub writes a placeholder
    // MEMORY.md so the rest of installation can proceed normally.
    let outcome = migration::migrate_memory(install, &opts, StubLlmFn).await;

    match &outcome {
        MemoryMigrationOutcome::Completed { dest_path } => {
            println!(
                "  {} Memory migrated → {}",
                style("✓").green().bold(),
            ...
            println!(
                "  {} {}",
                style("!").yellow(),
                style("Note: migration used a stub LLM. Run `nzc migrate-memory` with a real provider to regenerate.")
                    .dim()
            );
        }
        MemoryMigrationOutcome::Failed { reason, source_files } => {
            println!(
            ...
            println!(
                "  {} Source files preserved at original location. Run `nzc migrate-memory` to retry.",
                style("›").cyan()
            );
            if let Some(first) = source_files.first() {
...
/// Apply the channel assignments from migration to the freshly-built ChannelsConfig.
///
/// For channels assigned to NZC, attempts to pull credential fields from the
/// OpenClaw config snippet into the NZC channel config.  Channels assigned to
/// OpenClaw or Unassigned are left as-is (user can configure them in step 4).
fn setup_channels_with_migration(
    mut channels: ChannelsConfig,
    migration: &MigrationWizardResult,
) -> Result<ChannelsConfig> {
    for assignment in &migration.channel_assignments {
        if assignment.owner != ChannelOwner::Nzc {
            continue;
        }
        apply_channel_from_openclaw(&mut channels, &assignment.channel)?;
    }
    Ok(channels)
}

/// Pull credential fields from an OpenClaw channel config snippet into NZC's
/// `ChannelsConfig`.
///
/// Only applies fields that are present and non-empty in the OpenClaw snippet.
/// Never overwrites a field that the user has already configured.
fn apply_channel_from_openclaw(
    channels: &mut ChannelsConfig,
    detected: &migration::DetectedChannel,
) -> Result<()> {
    ...
    match detected.name.as_str() {
        "telegram" => {
            if channels.telegram.is_none() {
                let token = snippet
                    .get("botToken")
                    .or_else(|| snippet.get("bot_token"))
                    .or_else(|| snippet.get("token"))
                    .and_then(|v| v.as_str())
                    .unwrap_or("")
                    .to_string();

                let allow_from: Vec<String> = snippet
                    .get("allowFrom")
                    .or_else(|| snippet.get("allow_from"))
                ...
                if !token.is_empty() {
                    ...
                    println!(
                        "  {} Telegram credentials imported from OpenClaw.",
                        style("✓").green()
                    );
                }
            }
            ...
            if channels.discord.is_none() {
                ...
                if !token.is_empty() {
                    channels.discord = Some(DiscordConfig {
                        ...
                        guild_id: snippet
                            .get("guildId")
                            .or_else(|| snippet.get("guild_id"))
                            .and_then(|v| v.as_str())
                            .map(|s| s.to_string()),
                        allowed_users: snippet
                            .get("allowFrom")
                            .or_else(|| snippet.get("allow_from"))
                            .and_then(|v| v.as_array())
                            .map(|arr| {
                    ...
                    println!(
                        "  {} Discord credentials imported from OpenClaw.",
                        style("✓").green()
                    );
                }
            }
        ...
        "signal" => {
            if channels.signal.is_none() {
                let account = snippet
                    .get("account")
                    .or_else(|| snippet.get("phoneNumber"))
                    .or_else(|| snippet.get("phone_number"))
                    .and_then(|v| v.as_str())
                    .unwrap_or("")
                    .to_string();

                if !account.is_empty() {
                    // Signal also needs an http_url — use a sensible default.
                    let http_url = snippet
                        .get("httpUrl")
                        .or_else(|| snippet.get("http_url"))
                        .or_else(|| snippet.get("signalCliUrl"))
                        .and_then(|v| v.as_str())
                        .unwrap_or("http://127.0.0.1:8686")
                        .to_string();

                    channels.signal = Some(SignalConfig {
                        account,
                        http_url,
                        group_id: None,
                        allowed_from: snippet
                            .get("allowFrom")
                            .or_else(|| snippet.get("allow_from"))
                            .and_then(|v| v.as_array())
                            .map(|arr| {
                    ...
                    println!(
                        "  {} Signal account imported from OpenClaw.",
                        style("✓").green()
                    );
                }
            }
        ...
        "matrix" => {
            if channels.matrix.is_none() {
                // Matrix creds are complex — just note them, let user configure manually.
                println!(
                    "  {} Matrix assigned to NZC, but credentials require manual configuration (complex auth).",
                    style("!").yellow()
                );
            }
        }
        "whatsapp" => {
            // WhatsApp (Baileys) sessions are not portable — always re-link.
            println!(
                "  {} WhatsApp assigned to NZC. Note: session auth cannot be migrated — you will need to re-link.",
                style("!").yellow()
            );
        }
        _ => {}
