            ...
            match msg.role.as_str() {
                ...
                "assistant" => {
                    if let Some(blocks) = Self::parse_assistant_tool_call_message(&msg.content) {
                        ...
                        // Fix 2: skip empty/whitespace-only assistant text blocks
                        native_messages.push(NativeMessage {
                            role: "assistant".to_string(),
                ...
                "tool" => {
                    let tool_msg = if let Some(tr) = Self::parse_tool_result_message(&msg.content) {
                        ...
                        // Fix 2: skip empty/whitespace-only tool fallback text blocks
                        NativeMessage {
                            role: "user".to_string(),
                            content: vec![NativeContentOut::Text {
                                text: msg.content.clone(),
                    ...
                    // Fix 1: Tool results map to role "user"; merge consecutive ones
                ...
                _ => {
                    // User messages: skip empty/whitespace-only text (Fix 2).
                    // Also merge into previous user message if present, e.g.
                    // when a user message immediately follows tool results
                    // which are also role "user" in Anthropic's format (Fix 1).
                    if !msg.content.trim().is_empty() {
                        let content_block = NativeContentOut::Text {
                            text: msg.content.clone(),
                            cache_control: None,
                        };
...
mod tests {
    ...
    fn apply_auth_uses_bearer_and_beta_for_setup_tokens() {
        ...
        assert_eq!(
            ...
            Some("Bearer sk-ant-oat01-EXAMPLE-TOKEN")
        );
        assert_eq!(
            request
                .headers()
