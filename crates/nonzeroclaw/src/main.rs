...
Examples:
  ...
  zeroclaw gateway get-paircode       # show pairing code")]
    Gateway {
        #[command(subcommand)]
        gateway_command: Option<nonzeroclaw::GatewayCommands>,
    },
...
Examples:
  ...
  zeroclaw hardware info --chip STM32F401RETx")]
    Hardware {
        #[command(subcommand)]
        hardware_command: nonzeroclaw::HardwareCommands,
    },
...
Examples:
  ...
  zeroclaw peripheral flash-nucleo")]
    Peripheral {
        #[command(subcommand)]
        peripheral_command: nonzeroclaw::PeripheralCommands,
    },
    ...
    match cli.command {
        ...
        Commands::Gateway { gateway_command } => {
            match gateway_command {
                Some(nonzeroclaw::GatewayCommands::Restart { port, host }) => {
                ...
                Some(nonzeroclaw::GatewayCommands::GetPaircode { new }) => {
                ...
                Some(nonzeroclaw::GatewayCommands::Start { port, host }) => {
                    let (port, host) = resolve_gateway_addr(&config, port, host);
                    log_gateway_start(&host, port);
                    Box::pin(gateway::run_gateway(&host, port, config)).await
                }
        ...
        Commands::Plugin { plugin_command } => match plugin_command {
            PluginCommands::List => {
                let host = nonzeroclaw::plugins::host::PluginHost::new(&config.workspace_dir)?;
            ...
            PluginCommands::Install { source } => {
                let mut host = nonzeroclaw::plugins::host::PluginHost::new(&config.workspace_dir)?;
                host.install(&source)?;
