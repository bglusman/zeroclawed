...
struct ChannelRuntimeContext {
    ...
    /// Whether to forward tool-call notifications to the channel.
    /// When `false`, tool calls are suppressed from channel output (server-side
    /// logs remain unaffected). Currently a no-op stub — wire into a future
    /// ChannelNotifyObserver when that mechanism is added.
    /// Backport of upstream zeroclaw commit f900d70.
    #[allow(dead_code)]
