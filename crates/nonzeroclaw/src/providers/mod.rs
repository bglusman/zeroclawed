...
fn create_provider_with_url_and_options(
    ...
    match name {
        ...
        // NONZEROCLAW_PROVIDER_URL env var overrides the config value at runtime
        // without requiring config.toml changes (useful for container deployments,
        // or debugging endpoint changes like the kimi-proxy port switch).
        // Backport of upstream zeroclaw commit cabd3de, renamed for our binary.
        "ollama" => {
            let env_url = std::env::var("NONZEROCLAW_PROVIDER_URL").ok();
...
mod tests {
    ...
    /// NONZEROCLAW_PROVIDER_URL env var overrides api_url for Ollama.
    ///
    /// Backport of upstream zeroclaw commit cabd3de (renamed from ZEROCLAW_PROVIDER_URL).
    #[test]
    fn env_provider_url_overrides_api_url() {
        std::env::set_var("NONZEROCLAW_PROVIDER_URL", "http://env-ollama:11434");
        ...
        std::env::remove_var("NONZEROCLAW_PROVIDER_URL");
    }
