# PolyClaw Multi-Target Installer — Implementation Notes

_Session 4 of the polyclaw-mono sprint. Written for the Opus review session._

---

## What Was Implemented

### Module structure: `crates/polyclaw/src/install/`
...
### `apply_remote_config` (executor.rs: `apply_polyclaw_marker`)

**This is the most important stub.** Currently writes a `_polyclaw_registered: true` marker to the remote config. Production implementation should:
...
- Possibly add a named `polyclaw` plugin entry (depends on what OpenClaw schema version supports)
...
- Add a `[polyclaw]` upstream section with the PolyClaw endpoint + token
...
2. Write them to `~/.polyclaw/config.toml` (or the specified config path)
...
## Shared Structs Between NZC and PolyClaw
...
- `OpenClawInstallation` — needed by PolyClaw installer to read OpenClaw config
...
### Proposed shared crate: `crates/polyclaw-shared` or `crates/claw-types`

Moving these to a shared crate avoids the circular dependency (polyclaw can't depend on nonzeroclaw). Both crates would then depend on `claw-types`.

The PolyClaw installer currently has its own minimal `strip_json_comments_simple` (in executor.rs) — this is a stopgap. The real one from migration.rs is better (handles edge cases, tested).
...
2. **Shared crate extraction** — `OpenClawInstallation`, `DetectedChannel`, `parse_json5_relaxed` into a crate both NZC and PolyClaw can use.

3. **`--yes` flag propagation** — wire through to suppress `Confirm` dialogs in scripted runs.

4. **Config write-back** — the wizard collects channel routing assignments but doesn't persist them to `~/.polyclaw/config.toml`. This needs implementing.
