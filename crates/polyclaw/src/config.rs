...
//!
//! Reads from `~/.polyclaw/config.toml`. Supports the full PolyConfig v2 schema
//! as defined in the PolyClaw v2 spec (Section 3).
...
/// Top-level PolyClaw configuration.
#[derive(Debug, Clone, Deserialize, Serialize)]
pub struct PolyConfig {
    pub polyclaw: PolyHeader,
...
pub struct ChannelConfig {
    ...
    /// URL path PolyClaw registers for incoming WhatsApp webhooks.
...
/// Load the PolyConfig from `~/.polyclaw/config.toml`.
...
/// Returns the canonical config file path: `~/.polyclaw/config.toml`.
pub fn config_path() -> Result<PathBuf> {
    let home = home::home_dir().context("could not determine home directory")?;
    Ok(home.join(".polyclaw").join("config.toml"))
}
...
[[channels]]
kind = "telegram"
bot_token_file = "~/.polyclaw/secrets/telegram-token"
enabled = true
...
"#;
    ...
    fn test_parse_sample_config() {
        ...
        assert_eq!(cfg.polyclaw.version, 2);
    ...
    fn test_expand_tilde() {
        let p = expand_tilde("~/.polyclaw/secrets/telegram-token");
        assert!(p.to_string_lossy().contains(".polyclaw"));
    ...
    fn test_version_field() {
        let cfg: PolyConfig = toml::from_str(SAMPLE_CONFIG).expect("parse failed");
        assert_eq!(cfg.polyclaw.version, 2, "must be version 2");
    }
