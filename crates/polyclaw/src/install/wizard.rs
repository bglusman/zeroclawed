//! Interactive TUI wizard for the PolyClaw installer.
//!
//! Launched when `polyclaw install` is run with no CLI flags. Guides the
//! operator through configuring PolyClaw to route messages to one or more
//! downstream claws.
//!
//! # Wizard steps
//!
//! 1. **PolyClaw host** — where is PolyClaw running? (host, SSH key)
...
//! 4. **Channel routing** — for each PolyClaw-owned channel, which claw handles it?
...
use super::{
    ...
    model::{ClawKind, ClawTarget, InstallTarget, PolyClawTarget, WebhookFormat},
...
const WIZARD_BANNER: &str = r#"
  ...
  ║  Route messages from one PolyClaw to many claws          ║
  ╚══════════════════════════════════════════════════════════╝
"#;
...
pub async fn run_wizard() -> Result<()> {
    ...
    if !<std::io::Stdin as IsTerminal>::is_terminal(&std::io::stdin()) {
        bail!(
            "interactive wizard requires a TTY — use --polyclaw-host / --claw flags for non-interactive mode"
        );
    }

    println!("{}", style(WIZARD_BANNER).cyan().bold());

    // ── Step 1: PolyClaw host ─────────────────────────────────────────────
    println!("{}", style("Step 1/8 — PolyClaw Host").bold().underlined());
    ...
    let args = InstallArgs {
        polyclaw_host: Some(target.polyclaw.host.clone()),
        polyclaw_key: target.polyclaw.ssh_key.clone(),
        ..Default::default()
    };
...
fn collect_polyclaw_host() -> Result<PolyClawTarget> {
    println!("  Where is PolyClaw running?");
    println!("  (Enter 'local' if PolyClaw runs on this machine)");

    let host: String = Input::new()
        .with_prompt("  PolyClaw host (user@hostname or 'local')")
        .interact_text()
        .context("failed to read PolyClaw host")?;
    ...
    Ok(PolyClawTarget { host, ssh_key })
}
...
fn collect_channel_routing(target: &InstallTarget) -> Result<Vec<ChannelRouting>> {
    println!("  For each PolyClaw-owned channel, choose which claw handles it.");
...
fn display_install_review(target: &InstallTarget) {
    println!("  PolyClaw host: {}", style(&target.polyclaw.host).bold());
