//! Shared migration types for the PolyClaw installer.
//!
//! These types mirror the canonical definitions in
//! `crates/nonzeroclaw/src/onboard/migration.rs`.  They are duplicated here
//! because `polyclaw` does not (yet) depend on `nonzeroclaw` — doing so would
...
//! Both `nonzeroclaw` and `polyclaw` should depend on it.  This eliminates the
...
/// Everything PolyClaw's installer knows about an existing OpenClaw installation.
///
...
/// Who should own a channel after PolyClaw takes over routing.
///
/// Mirrors `nonzeroclaw::onboard::migration::ChannelOwner`.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum ChannelOwner {
    /// PolyClaw routes this channel to one of its configured claws.
    PolyClaw,
...
impl std::fmt::Display for ChannelOwner {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ChannelOwner::PolyClaw => write!(f, "PolyClaw"),
...
pub struct ChannelAssignment {
    ...
    /// Which PolyClaw claw handles this channel (if `owner == PolyClaw`).
...
mod tests {
    ...
    fn channel_owner_display() {
        assert_eq!(ChannelOwner::PolyClaw.to_string(), "PolyClaw");
    ...
    fn channel_assignment_fields() {
        ...
        let assignment = ChannelAssignment {
            channel: ch,
            owner: ChannelOwner::PolyClaw,
            assigned_claw: Some("librarian".into()),
        };
        assert_eq!(assignment.owner, ChannelOwner::PolyClaw);
