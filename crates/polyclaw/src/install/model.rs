//! Core data model for the PolyClaw multi-target installer.
//!
...
//! - All other adapters → installer just registers the endpoint in PolyClaw's
...
/// How PolyClaw dispatches messages to a downstream claw, and whether the
...
impl ClawKind {
    ...
    /// All other adapters are registered in PolyClaw's config and health-checked,
...
/// A downstream claw that PolyClaw will route messages to.
///
...
pub struct ClawTarget {
    /// Friendly name used in PolyClaw config and logs (e.g. `"librarian"`).
    pub name: String,
    /// How PolyClaw dispatches to this claw and whether remote config is possible.
    ...
    /// HTTP endpoint PolyClaw sends messages to (empty for `Cli` adapters).
    pub endpoint: String,
}
...
/// Where PolyClaw itself is running (may be local or remote).
#[derive(Debug, Clone)]
pub struct PolyClawTarget {
    /// SSH target for PolyClaw's host (`user@hostname`).
    pub host: String,
    /// SSH private key for PolyClaw's host.
    pub ssh_key: Option<PathBuf>,
}
...
/// Top-level install descriptor: where PolyClaw lives + all downstream claws.
#[derive(Debug, Clone)]
pub struct InstallTarget {
    /// PolyClaw host metadata.
    pub polyclaw: PolyClawTarget,
