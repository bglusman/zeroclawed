...
//!
//! - Session history is maintained across PolyClaw turns (stable `sessionKey`
...
struct HooksAgentRequest<'a> {
    ...
    ///
    /// Set to `false` — PolyClaw receives the response as the HTTP reply and
...
/// running when the 200 arrives.  PolyClaw needs a separate poll or inline
...
impl OpenClawNativeAdapter {
    ...
    ///
    /// Format: `"polyclaw:{agent_id}:{sender}"` — e.g. `"polyclaw:librarian:brian"`.
    ///
    /// The `polyclaw:` prefix should be listed in `hooks.allowedSessionKeyPrefixes`
    /// on the OpenClaw side to allow caller-supplied session keys.
    pub fn session_key_for(&self, sender: &str) -> String {
        format!("polyclaw:{}:{}", self.agent_id, sender)
    }
...
impl AgentAdapter for OpenClawNativeAdapter {
    ...
    async fn dispatch_with_context(
        ...
        let body = HooksAgentRequest {
            ...
            // the response inline.  PolyClaw then routes it to the channel.
            deliver: false,
        ...
        if !status.is_success() {
            ...
            if status == reqwest::StatusCode::BAD_REQUEST && body_text.contains("sessionKey") {
                warn!(
                    ...
                    hooks.allowedSessionKeyPrefixes=[\"polyclaw:\"] \
...
mod tests {
    ...
    fn test_session_key_for_sender() {
        let a = make_adapter(19100);
        assert_eq!(a.session_key_for("brian"), "polyclaw:librarian:brian");
        assert_eq!(a.session_key_for("renee"), "polyclaw:librarian:renee");
    }

    #[test]
    fn test_default_session_key() {
        let a = make_adapter(19100);
        assert_eq!(a.default_session_key(), "polyclaw:librarian");
    }
    ...
    fn test_session_key_format_uses_agent_id() {
        ...
        assert_eq!(a.session_key_for("alice"), "polyclaw:custodian:alice");
    }
    ...
    fn test_hooks_agent_request_serialization() {
        let req = HooksAgentRequest {
            message: "hello",
            name: Some("PolyClaw"),
            session_key: Some("polyclaw:librarian:brian".to_string()),
            deliver: false,
            agent_id: Some("librarian"),
        };
        let json = serde_json::to_value(&req).unwrap();
        assert_eq!(json["message"], "hello");
        assert_eq!(json["name"], "PolyClaw");
        assert_eq!(json["sessionKey"], "polyclaw:librarian:brian");
    ...
    async fn test_openclaw_native_adapter_passes_session_key() {
        ...
        assert!(
            req_text.contains("polyclaw:librarian:brian"),
            "expected session key 'polyclaw:librarian:brian' in request body, got:\n{}",
            req_text
        );
    ...
    async fn test_openclaw_native_maintains_session_across_turns() {
        ...
        assert_eq!(
            keys[0], "polyclaw:librarian:brian",
    ...
    async fn test_openclaw_native_forwards_sender_identity() {
        ...
        assert!(
            req_text.contains("polyclaw:librarian:renee"),
