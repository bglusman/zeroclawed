...
//! at the WA wire level) lives in NonZeroClaw's `whatsapp-web` feature. PolyClaw
//! acts as a routing sidecar:
//!
//! ```text
//! WA user  →  NZC (wa-rs session)  →  POST /webhooks/whatsapp  →  PolyClaw
//!                                                                      │
//!                                          identity resolution         │
//!                                          agent dispatch              │
//!                                                                      ↓
//! WA user  ←  NZC (wa-rs session)  ←  POST /tools/invoke  ←  PolyClaw reply
//! ```
//!
...
//! PolyClaw normalises it to E.164 format (`+15555550001`) before identity lookup.
//!
//! ## Config
//!
//! ```toml
...
//! # Webhook path PolyClaw registers (on the PolyClaw gateway HTTP server — see main.rs TODO)
...
/// routes through PolyClaw's identity/agent system and sends the reply via NZC.
///
...
fn verify_hmac_sha256(secret: &str, body: &[u8], sig_header: &str) -> bool {
    ...
    // The PolyClaw Cargo.toml already has reqwest (which pulls ring/rustls) but
...
mod tests {
    ...
    fn test_parse_valid_text_message() {
        ...
        assert_eq!(msgs[0].text, "Hello PolyClaw!");
