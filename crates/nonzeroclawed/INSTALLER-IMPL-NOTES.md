# NonZeroClawed Multi-Target Installer — Implementation Notes

_Session 4 of the nonzeroclawed sprint. Written for the Opus review session._

---

## What Was Implemented

### Module structure: `crates/nonzeroclawed/src/install/`
...
### `apply_remote_config` (executor.rs: `apply_nonzeroclawed_marker`)

**This is the most important stub.** Currently writes a `_nonzeroclawed_registered: true` marker to the remote config. Production implementation should:
...
- Possibly add a named `nonzeroclawed` plugin entry (depends on what OpenClaw schema version supports)
...
- Add a `[nonzeroclawed]` upstream section with the NonZeroClawed endpoint + token
...
2. Write them to `~/.nonzeroclawed/config.toml` (or the specified config path)
...
## Shared Structs Between NZC and NonZeroClawed
...
- `OpenClawInstallation` — needed by NonZeroClawed installer to read OpenClaw config
...
### Proposed shared crate: `crates/nonzeroclawed-shared` or `crates/claw-types`

Moving these to a shared crate avoids the circular dependency (nonzeroclawed can't depend on nonzeroclaw). Both crates would then depend on `claw-types`.

The NonZeroClawed installer currently has its own minimal `strip_json_comments_simple` (in executor.rs) — this is a stopgap. The real one from migration.rs is better (handles edge cases, tested).
...
2. **Shared crate extraction** — `OpenClawInstallation`, `DetectedChannel`, `parse_json5_relaxed` into a crate both NZC and NonZeroClawed can use.

3. **`--yes` flag propagation** — wire through to suppress `Confirm` dialogs in scripted runs.

4. **Config write-back** — the wizard collects channel routing assignments but doesn't persist them to `~/.nonzeroclawed/config.toml`. This needs implementing.
