...
//!
//! - Session history is maintained across NonZeroClawed turns (stable `sessionKey`
...
//!     allowedSessionKeyPrefixes: ["nonzeroclawed:"],
//!   }
//! }
//! ```
//!
...
struct HooksAgentRequest<'a> {
    ...
    ///
    /// Set to `false` — NonZeroClawed receives the response as the HTTP reply and
...
/// running when the 200 arrives.  NonZeroClawed needs a separate poll or inline
...
impl OpenClawNativeAdapter {
    ...
    ///
    /// Format: `"nonzeroclawed:{agent_id}:{sender}"` — e.g. `"nonzeroclawed:librarian:brian"`.
    ///
    /// The `nonzeroclawed:` prefix should be listed in `hooks.allowedSessionKeyPrefixes`
    /// on the OpenClaw side to allow caller-supplied session keys.
    pub fn session_key_for(&self, sender: &str) -> String {
        format!("nonzeroclawed:{}:{}", self.agent_id, sender)
    }
    ...
    pub fn default_session_key(&self) -> String {
        format!("nonzeroclawed:{}", self.agent_id)
    }
}
...
impl AgentAdapter for OpenClawNativeAdapter {
    ...
    async fn dispatch_with_context(
        ...
        let body = HooksAgentRequest {
            message: ctx.message,
            name: Some("NonZeroClawed"),
            session_key,
            // deliver = false: OpenClaw runs the loop synchronously and returns
            // the response inline.  NonZeroClawed then routes it to the channel.
            deliver: false,
        ...
        if !status.is_success() {
            ...
            if status == reqwest::StatusCode::BAD_REQUEST && body_text.contains("sessionKey") {
                warn!(
                    ...
                    hooks.allowedSessionKeyPrefixes=[\"nonzeroclawed:\"] \
...
mod tests {
    ...
    fn test_session_key_for_sender() {
        let a = make_adapter(19100);
        assert_eq!(a.session_key_for("brian"), "nonzeroclawed:librarian:brian");
        assert_eq!(a.session_key_for("renee"), "nonzeroclawed:librarian:renee");
    }

    #[test]
    fn test_default_session_key() {
        let a = make_adapter(19100);
        assert_eq!(a.default_session_key(), "nonzeroclawed:librarian");
    }
    ...
    fn test_session_key_format_uses_agent_id() {
        ...
        assert_eq!(a.session_key_for("alice"), "nonzeroclawed:custodian:alice");
    }
    ...
    fn test_hooks_agent_request_serialization() {
        let req = HooksAgentRequest {
            message: "hello",
            name: Some("NonZeroClawed"),
            session_key: Some("nonzeroclawed:librarian:brian".to_string()),
            deliver: false,
            agent_id: Some("librarian"),
        };
        let json = serde_json::to_value(&req).unwrap();
        assert_eq!(json["message"], "hello");
        assert_eq!(json["name"], "NonZeroClawed");
        assert_eq!(json["sessionKey"], "nonzeroclawed:librarian:brian");
    ...
    async fn test_openclaw_native_adapter_passes_session_key() {
        ...
        assert!(
            req_text.contains("nonzeroclawed:librarian:brian"),
            "expected session key 'nonzeroclawed:librarian:brian' in request body, got:\n{}",
            req_text
        );
    ...
    async fn test_openclaw_native_maintains_session_across_turns() {
        ...
        assert_eq!(
            keys[0], "nonzeroclawed:librarian:brian",
    ...
    async fn test_openclaw_native_forwards_sender_identity() {
        ...
        assert!(
            req_text.contains("nonzeroclawed:librarian:renee"),
