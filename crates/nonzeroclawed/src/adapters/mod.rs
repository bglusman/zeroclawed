...
//! to a downstream agent. NonZeroClawed's router calls `adapter.dispatch(text)` —
...
//!
//! ```no_run
//! use nonzeroclawed::adapters::{build_adapter, AgentAdapter};
...
/// NonZeroClawed router can send the approval notification to the user.
#[derive(Debug, Clone)]
...
pub struct DispatchContext<'a> {
    ...
    pub message: &'a str,
    /// Resolved identity name from NonZeroClawed (e.g. "brian", "renee").
    ...
    match agent.kind.as_str() {
        "openclaw-http" => {
            let token = agent
                ...
                .or_else(|| std::env::var("NONZEROCLAWED_AGENT_TOKEN").ok())
                .unwrap_or_default();
        ...
        // `hooks.allowRequestSessionKey = true` + `allowedSessionKeyPrefixes = ["nonzeroclawed:"]`
        ...
        "openclaw-native" => {
            ...
            // This allows a NonZeroClawed agent named "openclaw-max" to route to
            // OpenClaw's "david" agent without renaming the NonZeroClawed-side entry.
