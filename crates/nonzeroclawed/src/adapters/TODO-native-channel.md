...
(format: `nonzeroclawed:{agent_id}:{sender}`). Requires on the OpenClaw side:
...
- Commit: `fix(nonzeroclawed): native channel adapters for OpenClaw and NZC with session continuity`
- Files changed:
