...
//! config field, or the `NONZEROCLAWED_AGENT_TOKEN` env var as fallback.
//!
//! # Streaming
//!
...
//!
//! Benefits:
//! - No HTTP-level timeout on either side (NZC or NonZeroClawed)
//! - First-byte timeout of 30s is applied server-side in NZC
//! - NonZeroClawed waits until the SSE stream terminates (`data: [DONE]`)
...
///
/// For native command support, a NonZeroClawed channel plugin for OpenClaw is
...
impl AgentAdapter for OpenClawHttpAdapter {
    ...
    async fn dispatch_with_context(
        ...
        // history across NonZeroClawed-routed messages.  Without this, each call creates
        // a fresh OpenClaw session and the agent loses all prior context.
        // Key format: "nonzeroclawed-{agent_id}-{sender}" e.g. "nonzeroclawed-librarian-brian"
        let session_key = ctx
            .sender
            .map(|s| format!("nonzeroclawed-{}-{}", self.agent_id, s));
...
/// Metadata tracked by NonZeroClawed for a pending Clash approval.
///
...
/// Special:  {"approval_required": {...}} — NonZeroClawed notifies user, polls for result.
...
struct NzcWebhookRequest<'a> {
    message: &'a str,
    /// Resolved NonZeroClawed identity name (e.g. "brian"). Omitted when unknown.
    #[serde(skip_serializing_if = "Option::is_none")]
    sender: Option<&'a str>,
}
...
impl AgentAdapter for NzcHttpAdapter {
    ...
    async fn dispatch_with_context(
        ...
        // response.  NonZeroClawed stores the pending approval and returns
