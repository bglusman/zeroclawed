//! Local command handler for NonZeroClawed v3.
//!
...
/// Default state directory: `~/.nonzeroclawed/state/`.
fn default_state_dir() -> PathBuf {
    let home = std::env::var("HOME").unwrap_or_else(|_| "/root".to_string());
    PathBuf::from(home).join(".nonzeroclawed").join("state")
}
...
pub struct CommandHandler {
    ...
    /// Defaults to `~/.nonzeroclawed/state/`; overridable for tests via
...
impl CommandHandler {
    ...
    ///
    /// State is persisted to `~/.nonzeroclawed/state/`.  For test isolation, use
    ...
    pub async fn cmd_status_for_identity(&self, identity_id: &str) -> String {
        ...
        let version = self.config.nonzeroclawed.version;
    ...
    fn cmd_help(&self) -> String {
        [
            "NonZeroClawed v3 — available commands:",
    ...
    fn cmd_metrics(&self) -> String {
        ...
        format!("NonZeroClawed v3 metrics:\n  messages routed: {routed}\n  avg latency: {avg_latency}ms")
    }
}
...
mod tests {
    ...
    fn make_handler() -> CommandHandler {
        ...
        // `handle_switch` writes to the shared `~/.nonzeroclawed/state/active-agents.json`
    ...
    fn make_config() -> PolyConfig {
        PolyConfig {
            ...
            channels: vec![ChannelConfig {
                ...
                bot_token_file: Some("~/.nonzeroclawed/secrets/telegram-token".to_string()),
                enabled: true,
                ..Default::default()
            }],
            permissions: None,
