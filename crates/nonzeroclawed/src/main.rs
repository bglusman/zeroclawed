//! NonZeroClawed v3 — Rust agent gateway
//!
...
async fn main() -> Result<()> {
    ...
    fmt()
        .with_env_filter(EnvFilter::from_default_env().add_directive("nonzeroclawed=info".parse()?))
        .init();

    info!("NonZeroClawed v3 starting");
    ...
    info!(
        version = config.nonzeroclawed.version,
