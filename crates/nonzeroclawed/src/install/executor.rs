...
//!
//! Non-SSH adapters skip steps 1, 3, 7, 9; they just register in NonZeroClawed's
...
fn run_apply(
    ...
    if !claw.needs_ssh_config() {
        return StepResult {
            ...
            outcome: StepOutcome::Ok {
                detail: format!(
                    "no remote config needed for adapter '{}'; registered in NonZeroClawed config",
                    claw.adapter.kind_label()
                ),
            },
        };
...
fn describe_proposed_changes(claw: &ClawTarget) -> String {
    match &claw.adapter {
        ClawKind::OpenClawHttp => format!(
            "Will add NonZeroClawed webhook hook entry to openclaw.json on {} \
             (hooks.enabled = true, hooks.token = <generated>)",
            claw.host
        ),
        ClawKind::NzcNative => format!(
            "Will register NonZeroClawed as upstream router in NZC config on {}",
            claw.host
        ),
        ClawKind::OpenAiCompat { endpoint } => format!(
            "Will register endpoint '{}' in NonZeroClawed config (no remote changes)",
            endpoint
        ),
        ClawKind::Webhook { endpoint, format } => format!(
            "Will register webhook endpoint '{}' (format: {}) in NonZeroClawed config (no remote changes)",
            endpoint, format
        ),
        ClawKind::Cli { command } => format!(
            "Will register CLI command '{}' in NonZeroClawed config (no remote changes)",
            command
        ),
    }
}
...
fn describe_apply_changes(claw: &ClawTarget) -> String {
    match &claw.adapter {
        ClawKind::OpenClawHttp => format!(
            "would patch openclaw.json on {} to add NonZeroClawed hook entry",
            claw.host
        ),
        ClawKind::NzcNative => format!(
            "would patch NZC config on {} to register NonZeroClawed upstream",
            claw.host
        ),
        _ => format!("would register '{}' in NonZeroClawed config", claw.name),
    }
}
...
/// parses as JSON, injects the NonZeroClawed webhook hook entry under
/// `hooks.entries.nonzeroclawed`, serializes back to pretty JSON, writes via SSH,
/// and verifies the written file parses correctly.
///
/// For `NzcNative`: stub — adds a `[nonzeroclawed]` section to `config.toml`.
...
fn apply_remote_config(claw: &ClawTarget, deps: &ExecutorDeps) -> Result<String> {
    ...
    Ok(format!(
        "patched {} on {} — NonZeroClawed hook registered",
        config_path, claw.host
    ))
}

/// Patch `openclaw.json` to register NonZeroClawed as a webhook receiver.
///
/// Adds/updates `hooks.entries.nonzeroclawed` with `enabled`, `url`, and `token`.
...
fn patch_openclaw_config(
    ...
    // Upsert the NonZeroClawed entry.
    entries_obj.insert(
        claw_name.to_owned(),
        serde_json::json!({
            "enabled": true,
            "url": nonzeroclawed_endpoint,
            "token": token,
        }),
    );
...
///
/// Appends a minimal `[nonzeroclawed]` section if not already present.
/// Full TOML-aware patching is deferred to a follow-on session.
fn patch_nzc_config_stub(content: &str, claw_name: &str) -> String {
    if content.contains("[nonzeroclawed]") {
        return content.to_owned();
    }
    format!(
        "{}\n\n# NonZeroClawed registration — added by nonzeroclawed install\n\
         [nonzeroclawed]\n\
         registered = true\n\
         claw_name = {:?}\n",
        content, claw_name
...
mod tests {
    ...
    async fn full_install_summary_counts() {
        ...
        let target = InstallTarget {
            nonzeroclawed: super::super::model::NonZeroClawedTarget {
                host: "nonzeroclawed-host".into(),
                ssh_key: None,
            },
    ...
    fn patch_openclaw_config_adds_hook_entry() {
        ...
        let patched = patch_openclaw_config(input, "nonzeroclawed", "http://nonzeroclawed.host/hook")
            .expect("patch should succeed");

        let v: serde_json::Value = serde_json::from_str(&patched).unwrap();
        let entry = &v["hooks"]["entries"]["nonzeroclawed"];
        assert_eq!(entry["enabled"], serde_json::json!(true));
        assert_eq!(entry["url"], "http://nonzeroclawed.host/hook");
    ...
    fn patch_openclaw_config_enables_hooks() {
        ...
        let patched =
            patch_openclaw_config(input, "nonzeroclawed", "http://pc/hook").expect("should patch");
        let v: serde_json::Value = serde_json::from_str(&patched).unwrap();
        assert_eq!(v["hooks"]["entries"]["nonzeroclawed"]["enabled"], true);
    }

    /// patch_openclaw_config preserves existing token on re-run.
    #[test]
    fn patch_openclaw_config_preserves_existing_token() {
        let input = r#"{"hooks": {"enabled": true, "entries": {"nonzeroclawed": {"enabled": true, "url": "old", "token": "existing-tok"}}}}"#;
        let patched =
            patch_openclaw_config(input, "nonzeroclawed", "http://new/hook").expect("should patch");
        let v: serde_json::Value = serde_json::from_str(&patched).unwrap();
        // Token preserved; URL updated.
        assert_eq!(v["hooks"]["entries"]["nonzeroclawed"]["token"], "existing-tok");
        assert_eq!(v["hooks"]["entries"]["nonzeroclawed"]["url"], "http://new/hook");
    }

    /// patch_openclaw_config fails gracefully on invalid JSON.
    #[test]
    fn patch_openclaw_config_invalid_json_returns_error() {
        let result = patch_openclaw_config("{ not valid json", "nonzeroclawed", "http://pc/hook");
        assert!(result.is_err());
    }

    /// patch_nzc_config_stub appends [nonzeroclawed] section.
    #[test]
    fn patch_nzc_config_stub_appends_section() {
        let input = "[agent]\nname = \"librarian\"\n";
        let patched = patch_nzc_config_stub(input, "test-claw");
        assert!(patched.contains("[nonzeroclawed]"));
    ...
    fn patch_nzc_config_stub_idempotent() {
        ...
        assert_eq!(
            patched, input,
            "should not re-add [nonzeroclawed] if already present"
        );
    }
    ...
    async fn apply_remote_config_via_mock_writes_hooks_entry() {
        let claw = ClawTarget {
            name: "nonzeroclawed".into(),
            adapter: ClawKind::OpenClawHttp,
            host: "user@host".into(),
            ssh_key: Some(PathBuf::from("/keys/id_ed25519")),
            endpoint: "http://nonzeroclawed.host:18799/webhook".into(),
        };
        ...
        ssh.push_success(r#"{"version": "2026.3.13", "hooks": {"enabled": true, "entries": {"nonzeroclawed": {"enabled": true, "url": "http://nonzeroclawed.host:18799/webhook", "token": "abc123"}}}}"#);
    ...
    fn patch_openclaw_config_written_json_contains_hook() {
        ...
        let endpoint = "http://nonzeroclawed.internal:18799/hooks/nonzeroclawed";
        let patched =
            patch_openclaw_config(original, "nonzeroclawed", endpoint).expect("patch succeeds");
        ...
        assert!(
            entry.is_object(),
            "hooks.entries.nonzeroclawed must be an object"
        );
