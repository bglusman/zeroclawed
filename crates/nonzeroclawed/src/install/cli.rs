//! CLI argument parsing for `nonzeroclawed install`.
//!
//! Supports both interactive mode (no flags) and non-interactive mode
//! (`--nonzeroclawed-host` / one or more `--claw` flags).
//!
//! # `--claw` flag format
//!
...
use super::model::{ClawKind, ClawTarget, InstallTarget, NonZeroClawedTarget, WebhookFormat};

// ---------------------------------------------------------------------------
// InstallArgs
// ---------------------------------------------------------------------------

/// Parsed CLI arguments for `nonzeroclawed install`.
///
...
pub struct InstallArgs {
    /// `--nonzeroclawed-host user@host`
    pub nonzeroclawed_host: Option<String>,
    /// `--nonzeroclawed-key /path/to/key`
    pub nonzeroclawed_key: Option<PathBuf>,
...
impl InstallArgs {
    ...
    pub fn is_interactive(&self) -> bool {
        self.nonzeroclawed_host.is_none() && self.claw_specs.is_empty()
    }
}
...
pub fn parse_install_target(args: &InstallArgs) -> Result<InstallTarget> {
    let host = args
        .nonzeroclawed_host
        .clone()
        .context("--nonzeroclawed-host is required in non-interactive mode")?;

    let nonzeroclawed = NonZeroClawedTarget {
        host,
        ssh_key: args.nonzeroclawed_key.clone(),
    };
    ...
    Ok(InstallTarget { nonzeroclawed, claws })
}
...
mod tests {
    ...
    fn parse_install_target_full() {
        let args = InstallArgs {
            nonzeroclawed_host: Some("admin@10.0.0.1".into()),
            nonzeroclawed_key: Some(PathBuf::from("/keys/id_rsa")),
            claw_specs: vec![
        ...
        assert_eq!(target.nonzeroclawed.host, "admin@10.0.0.1");
    ...
    fn parse_install_target_missing_host_errors() {
        ...
        assert!(result.is_err());
        let msg = result.err().unwrap().to_string();
        assert!(msg.contains("--nonzeroclawed-host"), "got: {}", msg);
    }

    #[test]
    fn parse_install_target_no_claws_errors() {
        let args = InstallArgs {
            nonzeroclawed_host: Some("host".into()),
            claw_specs: vec![],
            ..Default::default()
        };
        let result = parse_install_target(&args);
    ...
    fn prop_parse_install_target_never_panics(tc: hegel::TestCase) {
        ...
        let args = InstallArgs {
            nonzeroclawed_host: if host.is_empty() {
                None
            } else {
                Some(host.clone())
            },
        ...
        if let Ok(target) = result {
            if !host.is_empty() {
                assert_eq!(
                    target.nonzeroclawed.host, host,
                    "nonzeroclawed host must roundtrip: input={:?}",
                    host
                );
            }
        }
