//! Core data model for the NonZeroClawed multi-target installer.
//!
...
//! - All other adapters → installer just registers the endpoint in NonZeroClawed's
...
/// How NonZeroClawed dispatches messages to a downstream claw, and whether the
...
impl ClawKind {
    ...
    /// All other adapters are registered in NonZeroClawed's config and health-checked,
...
/// A downstream claw that NonZeroClawed will route messages to.
///
...
pub struct ClawTarget {
    /// Friendly name used in NonZeroClawed config and logs (e.g. `"librarian"`).
    pub name: String,
    /// How NonZeroClawed dispatches to this claw and whether remote config is possible.
    ...
    /// HTTP endpoint NonZeroClawed sends messages to (empty for `Cli` adapters).
    pub endpoint: String,
}
...
/// Where NonZeroClawed itself is running (may be local or remote).
#[derive(Debug, Clone)]
pub struct NonZeroClawedTarget {
    /// SSH target for NonZeroClawed's host (`user@hostname`).
    pub host: String,
    /// SSH private key for NonZeroClawed's host.
    pub ssh_key: Option<PathBuf>,
}
...
/// Top-level install descriptor: where NonZeroClawed lives + all downstream claws.
#[derive(Debug, Clone)]
pub struct InstallTarget {
    /// NonZeroClawed host metadata.
    pub nonzeroclawed: NonZeroClawedTarget,
