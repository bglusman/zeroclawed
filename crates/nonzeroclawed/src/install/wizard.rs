//! Interactive TUI wizard for the NonZeroClawed installer.
//!
//! Launched when `nonzeroclawed install` is run with no CLI flags. Guides the
//! operator through configuring NonZeroClawed to route messages to one or more
//! downstream claws.
//!
//! # Wizard steps
//!
//! 1. **NonZeroClawed host** — where is NonZeroClawed running? (host, SSH key)
...
//! 4. **Channel routing** — for each NonZeroClawed-owned channel, which claw handles it?
...
use super::{
    ...
    model::{ClawKind, ClawTarget, InstallTarget, NonZeroClawedTarget, WebhookFormat},
...
const WIZARD_BANNER: &str = r#"
  ╔══════════════════════════════════════════════════════════╗
  ║         NonZeroClawed Multi-Target Installer                  ║
  ║  Route messages from one NonZeroClawed to many claws          ║
  ╚══════════════════════════════════════════════════════════╝
"#;
...
pub async fn run_wizard() -> Result<()> {
    ...
    if !<std::io::Stdin as IsTerminal>::is_terminal(&std::io::stdin()) {
        bail!(
            "interactive wizard requires a TTY — use --nonzeroclawed-host / --claw flags for non-interactive mode"
        );
    }

    println!("{}", style(WIZARD_BANNER).cyan().bold());

    // ── Step 1: NonZeroClawed host ─────────────────────────────────────────────
    println!("{}", style("Step 1/8 — NonZeroClawed Host").bold().underlined());
    let nonzeroclawed = collect_nonzeroclawed_host()?;
    ...
    let target = InstallTarget { nonzeroclawed, claws };
    ...
    let args = InstallArgs {
        nonzeroclawed_host: Some(target.nonzeroclawed.host.clone()),
        nonzeroclawed_key: target.nonzeroclawed.ssh_key.clone(),
        ..Default::default()
    };
    ...
    println!(
        "\n{}",
        style("All claws installed successfully! NonZeroClawed is ready.")
            .green()
            .bold()
    );
    Ok(())
...
fn collect_nonzeroclawed_host() -> Result<NonZeroClawedTarget> {
    println!("  Where is NonZeroClawed running?");
    println!("  (Enter 'local' if NonZeroClawed runs on this machine)");

    let host: String = Input::new()
        .with_prompt("  NonZeroClawed host (user@hostname or 'local')")
        .interact_text()
        .context("failed to read NonZeroClawed host")?;
    ...
    Ok(NonZeroClawedTarget { host, ssh_key })
}
...
fn collect_claws() -> Result<Vec<ClawTarget>> {
    ...
    println!("  Add the downstream claws NonZeroClawed should route messages to.");
    println!(
        "  {} — knows the config format, can SSH in and apply changes safely.",
        style("NZC / OpenClaw").bold()
    );
    println!(
        "  {} — registered in NonZeroClawed config; no remote config management.",
...
fn collect_channel_routing(target: &InstallTarget) -> Result<Vec<ChannelRouting>> {
    println!("  For each NonZeroClawed-owned channel, choose which claw handles it.");
...
fn display_install_review(target: &InstallTarget) {
    println!("  NonZeroClawed host: {}", style(&target.nonzeroclawed.host).bold());
...
mod tests {
    ...
    fn display_install_review_no_panic() {
        let target = InstallTarget {
            nonzeroclawed: NonZeroClawedTarget {
