//! Channel adapters for NonZeroClawed v3.
//!
...
//! NonZeroClawed listens for incoming webhook POSTs (forwarded from NZC) and sends
//! replies back via NZC's /tools/invoke API.  The QR pairing happens in NZC;
//! NonZeroClawed only handles identity routing and agent dispatch.
//!
//! Signal follows the same webhook receiver pattern as WhatsApp, but uses
//! OpenClaw's native Signal support. NonZeroClawed receives webhooks from OpenClaw
