//! Matrix channel adapter for NonZeroClawed v3.
//!
//! ## Architecture
//!
...
//! Matrix user is also matched against the NonZeroClawed identity table via the
...
pub async fn run(
    ...
    if has_matrix {
        tracing::warn!(
            "Matrix channel is enabled in config but NonZeroClawed was built without \
...
mod inner {
    ...
    pub async fn run(
        ...
        // Store E2EE keys in ~/.nonzeroclawed/state/matrix/
        info!("Matrix: preparing store directory...");
        let store_dir = {
            let home = home::home_dir();
            home.map(|h| h.join(".nonzeroclawed").join("state").join("matrix"))
        };
