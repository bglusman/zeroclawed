//! WhatsApp channel adapter for NonZeroClawed v3.
//!
//! ## Architecture
//!
...
//! at the WA wire level) lives in NonZeroClaw's `whatsapp-web` feature. NonZeroClawed
//! acts as a routing sidecar:
//!
//! ```text
//! WA user  →  NZC (wa-rs session)  →  POST /webhooks/whatsapp  →  NonZeroClawed
//!                                                                      │
//!                                          identity resolution         │
//!                                          agent dispatch              │
//!                                                                      ↓
//! WA user  ←  NZC (wa-rs session)  ←  POST /tools/invoke  ←  NonZeroClawed reply
//! ```
//!
...
//! NonZeroClawed normalises it to E.164 format (`+15555550001`) before identity lookup.
//!
//! ## Config
//!
//! ```toml
...
//! # Webhook path NonZeroClawed registers (on the NonZeroClawed gateway HTTP server — see main.rs TODO)
...
/// routes through NonZeroClawed's identity/agent system and sends the reply via NZC.
///
...
fn verify_hmac_sha256(secret: &str, body: &[u8], sig_header: &str) -> bool {
    ...
    // The NonZeroClawed Cargo.toml already has reqwest (which pulls ring/rustls) but
...
mod tests {
    ...
    fn make_test_config() -> Arc<PolyConfig> {
        Arc::new(PolyConfig {
            nonzeroclawed: PolyHeader { version: 2 },
    ...
    fn test_parse_valid_text_message() {
        ...
        assert_eq!(msgs[0].text, "Hello NonZeroClawed!");
