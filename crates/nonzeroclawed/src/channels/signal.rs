//! Signal channel adapter for NonZeroClawed v3.
//!
//! ## Architecture
//!
...
//! `action = "send"` and `channel = "signal"`. NonZeroClawed acts as a routing
//! sidecar:
//!
//! ```text
//! Signal user  →  OpenClaw (signal-web)  →  POST /webhooks/signal  →  NonZeroClawed
//!                                                                          │
//!                                              identity resolution         │
//!                                              agent dispatch              │
//!                                                                          ↓
//! Signal user  ←  OpenClaw (signal-web)  ←  POST /tools/invoke  ←  NonZeroClawed reply
//! ```
//!
...
//! NonZeroClawed uses it directly for identity lookup.
//!
//! ## Config
//!
//! ```toml
...
//! # Webhook path NonZeroClawed registers (on the NonZeroClawed gateway HTTP server)
...
/// routes through NonZeroClawed's identity/agent system and sends the reply via
