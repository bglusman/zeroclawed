...
```
WA user  ──→  NZC (wa-rs session on .210)  ──→  POST /webhooks/whatsapp  ──→  ZeroClawed
                                                                                   │
                                                  identity resolution              │
                                                  agent dispatch (Librarian/NZC)   │
                                                                                   ↓
WA user  ←──  NZC (wa-rs session on .210)  ←──  POST /tools/invoke  ←──  ZeroClawed reply
```

## Step 1: ZeroClawed config (`/root/.zeroclawed/config.toml` on 10.0.0.10)
...
# ZeroClawed sends replies by POSTing to {nzc_endpoint}/tools/invoke.
# If ZeroClawed is co-located with OpenClaw on .210, use 127.0.0.1.
# If ZeroClawed is on .229 (Librarian), point to .210 where NZC runs.
nzc_endpoint  = "http://127.0.0.1:18789"
nzc_auth_token = "REPLACE_WITH_AUTH_TOKEN"

# ZeroClawed's own webhook listener — NZC will POST incoming WA messages here.
...
NZC needs to forward incoming WA messages to ZeroClawed instead of processing them
...
# Forward incoming messages to ZeroClawed's webhook
webhook_forward_url    = "http://127.0.0.1:18795/webhooks/whatsapp"
# webhook_forward_secret = "change-me-to-a-random-secret"  # Must match ZeroClawed's webhook_secret
...
```
WA  →  NZC (wa-rs receives)  →  NZC forwards to ZeroClawed webhook
                                        ↓
                               ZeroClawed routes → Agent
                                        ↓
                               ZeroClawed POSTs reply to NZC /tools/invoke
                                        ↓
                                NZC sends WA reply
```
...
Ensure port 18795 is open on the ZeroClawed host (or at least reachable from NZC).

If NZC and ZeroClawed are on the same host:
```
# No firewall changes needed — both use localhost
```

If NZC is on .210 and ZeroClawed is on .229:
```bash
...
ZeroClawed expects the standard WhatsApp Cloud API format:
...
The `from` field may omit the leading `+` — ZeroClawed normalises to E.164 format
...
```bash
# Check ZeroClawed webhook listener is up
