//! OpenClawChannelAdapter — bridge ZeroClawed to the OpenClaw zeroclawed plugin.
//!
//! This adapter posts inbound messages to OpenClaw at
//! `POST /zeroclawed/inbound` (gateway auth) and waits for a correlated callback on the
...
impl OpenClawChannelAdapter {
    ...
    fn session_key_for(&self, sender: &str) -> String {
        format!("zeroclawed:{}:{}", self.openclaw_agent_id, sender)
    }
...
mod tests {
    ...
    async fn test_dispatch_sends_expected_inbound_payload() {
        ...
        assert_eq!(
            ...
            Some("hello from zeroclawed")
        );
        assert_eq!(
            body.get("sessionKey").and_then(|v| v.as_str()),
            Some("zeroclawed:main:brian")
        );
