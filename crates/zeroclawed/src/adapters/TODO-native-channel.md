...
(format: `zeroclawed:{agent_id}:{sender}`). Requires on the OpenClaw side:
...
- Commit: `fix(zeroclawed): native channel adapters for OpenClaw and NZC with session continuity`
- Files changed:
