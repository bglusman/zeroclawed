...
//! config field, or the `ZEROCLAWED_AGENT_TOKEN` env var as fallback.
//!
//! # Streaming
//!
...
//!
//! Benefits:
//! - No HTTP-level timeout on either side (NZC or ZeroClawed)
...
///
/// For native command support, a ZeroClawed channel plugin for OpenClaw is
...
impl AgentAdapter for OpenClawHttpAdapter {
    ...
    async fn dispatch_with_context(
        ...
        // history across ZeroClawed-routed messages.  Without this, each call creates
        // a fresh OpenClaw session and the agent loses all prior context.
        // Key format: "zeroclawed-{agent_id}-{sender}" e.g. "zeroclawed-librarian-brian"
        let session_key = ctx
            .sender
            .map(|s| format!("zeroclawed-{}-{}", self.agent_id, s));
...
/// Metadata tracked by ZeroClawed for a pending Clash approval.
///
...
/// Special:  {"approval_required": {...}} — ZeroClawed notifies user, polls for result.
...
impl AgentAdapter for NzcHttpAdapter {
    ...
    async fn dispatch_with_context(
        ...
        // response.  ZeroClawed stores the pending approval and returns
