...
/// Top-level ZeroClawed configuration.
#[derive(Debug, Clone, Deserialize, Serialize)]
pub struct PolyConfig {
    pub zeroclawed: PolyHeader,
...
pub struct AgentConfig {
    ...
    /// Per-agent API key / Bearer token. Overrides global `ZEROCLAWED_AGENT_TOKEN`.
...
pub struct ChannelConfig {
    ...
    /// ZeroClawed will POST reply messages to `{nzc_endpoint}/tools/invoke`.
    ...
    /// URL path ZeroClawed registers for incoming WhatsApp webhooks.
...
/// Returns the canonical config file path: `~/.zeroclawed/config.toml`.
pub fn config_path() -> Result<PathBuf> {
    let home = home::home_dir().context("could not determine home directory")?;
    Ok(home.join(".zeroclawed").join("config.toml"))
}
...
"#;
    ...
    fn test_parse_sample_config() {
        ...
        assert_eq!(cfg.zeroclawed.version, 2);
    ...
    fn test_expand_tilde() {
        let p = expand_tilde("~/.zeroclawed/secrets/telegram-token");
        assert!(p.to_string_lossy().contains(".zeroclawed"));
    ...
    fn test_version_field() {
        let cfg: PolyConfig = toml::from_str(SAMPLE_CONFIG).expect("parse failed");
        assert_eq!(cfg.zeroclawed.version, 2, "must be version 2");
    }
