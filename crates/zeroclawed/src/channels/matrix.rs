...
//! Matrix user is also matched against the ZeroClawed identity table via the
...
pub async fn run(
    ...
    if has_matrix {
        tracing::warn!(
            "Matrix channel is enabled in config but ZeroClawed was built without \
...
mod inner {
    ...
    pub async fn run(
        ...
        // Store E2EE keys in ~/.zeroclawed/state/matrix/
        info!("Matrix: preparing store directory...");
        let store_dir = {
            let home = home::home_dir();
            home.map(|h| h.join(".zeroclawed").join("state").join("matrix"))
        };
