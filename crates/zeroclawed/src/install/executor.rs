...
//!
//! Non-SSH adapters skip steps 1, 3, 7, 9; they just register in ZeroClawed's
...
fn describe_proposed_changes(claw: &ClawTarget) -> String {
    match &claw.adapter {
        ClawKind::OpenClawHttp => format!(
            "Will add ZeroClawed webhook hook entry to openclaw.json on {} \
             (hooks.enabled = true, hooks.token = <generated>)",
            claw.host
        ),
        ClawKind::NzcNative => format!(
            "Will register ZeroClawed as upstream router in NZC config on {}",
            claw.host
        ),
        ClawKind::OpenAiCompat { endpoint } => format!(
            "Will register endpoint '{}' in ZeroClawed config (no remote changes)",
            endpoint
        ),
        ClawKind::Webhook { endpoint, format } => format!(
            "Will register webhook endpoint '{}' (format: {}) in ZeroClawed config (no remote changes)",
            endpoint, format
        ),
        ClawKind::Cli { command } => format!(
            "Will register CLI command '{}' in ZeroClawed config (no remote changes)",
            command
        ),
    }
}
...
fn describe_apply_changes(claw: &ClawTarget) -> String {
    match &claw.adapter {
        ClawKind::OpenClawHttp => format!(
            "would patch openclaw.json on {} to add ZeroClawed hook entry",
            claw.host
        ),
        ClawKind::NzcNative => format!(
            "would patch NZC config on {} to register ZeroClawed upstream",
            claw.host
        ),
        _ => format!("would register '{}' in ZeroClawed config", claw.name),
    }
}
...
/// parses as JSON, injects the ZeroClawed webhook hook entry under
/// `hooks.entries.zeroclawed`, serializes back to pretty JSON, writes via SSH,
/// and verifies the written file parses correctly.
///
/// For `NzcNative`: stub — adds a `[zeroclawed]` section to `config.toml`.
...
fn apply_remote_config(claw: &ClawTarget, deps: &ExecutorDeps) -> Result<String> {
    ...
    Ok(format!(
        "patched {} on {} — ZeroClawed hook registered",
        config_path, claw.host
    ))
}

/// Patch `openclaw.json` to register ZeroClawed as a webhook receiver.
///
/// Adds/updates `hooks.entries.zeroclawed` with `enabled`, `url`, and `token`.
...
fn patch_openclaw_config(
    ...
    // Upsert the ZeroClawed entry.
    entries_obj.insert(
        claw_name.to_owned(),
        serde_json::json!({
            "enabled": true,
            "url": zeroclawed_endpoint,
            "token": token,
        }),
    );
...
///
/// Appends a minimal `[zeroclawed]` section if not already present.
/// Full TOML-aware patching is deferred to a follow-on session.
fn patch_nzc_config_stub(content: &str, claw_name: &str) -> String {
    if content.contains("[zeroclawed]") {
        return content.to_owned();
    }
    format!(
        "{}\n\n# ZeroClawed registration — added by zeroclawed install\n\
         [zeroclawed]\n\
         registered = true\n\
         claw_name = {:?}\n",
        content, claw_name
...
mod tests {
    ...
    async fn full_install_summary_counts() {
        ...
        let target = InstallTarget {
            zeroclawed: super::super::model::ZeroClawedTarget {
                host: "zeroclawed-host".into(),
                ssh_key: None,
            },
    ...
    fn patch_openclaw_config_adds_hook_entry() {
        ...
        let patched = patch_openclaw_config(input, "zeroclawed", "http://zeroclawed.host/hook")
            .expect("patch should succeed");

        let v: serde_json::Value = serde_json::from_str(&patched).unwrap();
        let entry = &v["hooks"]["entries"]["zeroclawed"];
        assert_eq!(entry["enabled"], serde_json::json!(true));
        assert_eq!(entry["url"], "http://zeroclawed.host/hook");
    ...
    fn patch_openclaw_config_enables_hooks() {
        ...
        let patched =
            patch_openclaw_config(input, "zeroclawed", "http://pc/hook").expect("should patch");
        let v: serde_json::Value = serde_json::from_str(&patched).unwrap();
        assert_eq!(v["hooks"]["entries"]["zeroclawed"]["enabled"], true);
    }

    /// patch_openclaw_config preserves existing token on re-run.
    #[test]
    fn patch_openclaw_config_preserves_existing_token() {
        let input = r#"{"hooks": {"enabled": true, "entries": {"zeroclawed": {"enabled": true, "url": "old", "token": "existing-tok"}}}}"#;
        let patched =
            patch_openclaw_config(input, "zeroclawed", "http://new/hook").expect("should patch");
        let v: serde_json::Value = serde_json::from_str(&patched).unwrap();
    ...
    fn patch_openclaw_config_invalid_json_returns_error() {
        let result = patch_openclaw_config("{ not valid json", "zeroclawed", "http://pc/hook");
        assert!(result.is_err());
    }

    /// patch_nzc_config_stub appends [zeroclawed] section.
    #[test]
    fn patch_nzc_config_stub_appends_section() {
        let input = "[agent]\nname = \"librarian\"\n";
        let patched = patch_nzc_config_stub(input, "test-claw");
        assert!(patched.contains("[zeroclawed]"));
    ...
    fn patch_nzc_config_stub_idempotent() {
        ...
        assert_eq!(
            patched, input,
            "should not re-add [zeroclawed] if already present"
        );
    }
    ...
    async fn apply_remote_config_via_mock_writes_hooks_entry() {
        let claw = ClawTarget {
            name: "zeroclawed".into(),
            adapter: ClawKind::OpenClawHttp,
            host: "user@host".into(),
            ssh_key: Some(PathBuf::from("/keys/id_ed25519")),
            endpoint: "http://zeroclawed.host:18799/webhook".into(),
        };
        ...
        ssh.push_success(r#"{"version": "2026.3.13", "hooks": {"enabled": true, "entries": {"zeroclawed": {"enabled": true, "url": "http://zeroclawed.host:18799/webhook", "token": "abc123"}}}}"#);
    ...
    fn patch_openclaw_config_written_json_contains_hook() {
        ...
        let endpoint = "http://zeroclawed.internal:18799/hooks/zeroclawed";
        let patched =
            patch_openclaw_config(original, "zeroclawed", endpoint).expect("patch succeeds");
        ...
        assert!(
            entry.is_object(),
            "hooks.entries.zeroclawed must be an object"
        );
