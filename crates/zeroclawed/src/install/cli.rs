//! CLI argument parsing for `zeroclawed install`.
//!
//! Supports both interactive mode (no flags) and non-interactive mode
//! (`--zeroclawed-host` / one or more `--claw` flags).
//!
//! # `--claw` flag format
//!
...
/// Parsed CLI arguments for `zeroclawed install`.
///
...
pub struct InstallArgs {
    /// `--zeroclawed-host user@host`
    pub zeroclawed_host: Option<String>,
    /// `--zeroclawed-key /path/to/key`
    pub zeroclawed_key: Option<PathBuf>,
...
impl InstallArgs {
    ...
    pub fn is_interactive(&self) -> bool {
        self.zeroclawed_host.is_none() && self.claw_specs.is_empty()
    }
}
...
pub fn parse_install_target(args: &InstallArgs) -> Result<InstallTarget> {
    let host = args
        .zeroclawed_host
        .clone()
        .context("--zeroclawed-host is required in non-interactive mode")?;

    let zeroclawed = ZeroClawedTarget {
        host,
        ssh_key: args.zeroclawed_key.clone(),
    };
    ...
    Ok(InstallTarget { zeroclawed, claws })
}
...
mod tests {
    ...
    fn parse_install_target_full() {
        ...
        assert_eq!(target.zeroclawed.host, "admin@10.0.0.1");
    ...
    fn parse_install_target_missing_host_errors() {
        ...
        assert!(result.is_err());
        let msg = result.err().unwrap().to_string();
        assert!(msg.contains("--zeroclawed-host"), "got: {}", msg);
    }

    #[test]
    fn parse_install_target_no_claws_errors() {
        let args = InstallArgs {
            zeroclawed_host: Some("host".into()),
            claw_specs: vec![],
            ..Default::default()
        };
        let result = parse_install_target(&args);
    ...
    fn prop_parse_install_target_never_panics(tc: hegel::TestCase) {
        ...
        let args = InstallArgs {
            zeroclawed_host: if host.is_empty() {
                None
            } else {
                Some(host.clone())
            },
        ...
        if let Ok(target) = result {
            if !host.is_empty() {
                assert_eq!(
                    target.zeroclawed.host, host,
                    "zeroclawed host must roundtrip: input={:?}",
                    host
                );
            }
        }
