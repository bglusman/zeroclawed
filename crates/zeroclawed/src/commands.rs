...
pub struct CommandHandler {
    ...
    /// Defaults to `~/.zeroclawed/state/`; overridable for tests via
...
impl CommandHandler {
    ...
    ///
    /// State is persisted to `~/.zeroclawed/state/`.  For test isolation, use
...
mod tests {
    ...
    fn make_handler() -> CommandHandler {
        ...
        // `handle_switch` writes to the shared `~/.zeroclawed/state/active-agents.json`
    ...
    fn make_config() -> PolyConfig {
        PolyConfig {
            ...
            channels: vec![ChannelConfig {
                ...
                bot_token_file: Some("~/.zeroclawed/secrets/telegram-token".to_string()),
                enabled: true,
                ..Default::default()
            }],
            permissions: None,
