# Vault Integration + PolyClaw Native OpenClaw Adapter — Planning Notes
...
2. **PolyClaw Native OpenClaw Adapter** — safe, versioned, installation-time integration with downstream OpenClaw instances
...
- NZC/PolyClaw owns the approval relay (it's the channel layer)
...
NZC/PolyClaw holds the session token only. Master password handled at unlock time.

### Phase 1: No Agent Access SDK Required

Use `bw` CLI subprocess for vault access first. Build the approval relay directly in PolyClaw.
...
## Workstream 2: PolyClaw Native OpenClaw Adapter

### Motivation
- PolyClaw needs to integrate with downstream OpenClaw instances (add channel, configure routing)
- This requires editing OpenClaw's config — which is **dangerous if done wrong**
- 2026-03-30: Librarian tried to add a PolyClaw channel adapter manually → broke config → gateway crashed → Custodian couldn't fix it → required manual recovery

### Core Principle
> **Config changes to a downstream claw must be initiated by PolyClaw's installer, not by the claw itself.**

The claw should never edit its own config in response to a PolyClaw integration request. PolyClaw owns that process.
...
```
polyclaw install-adapter --target <claw-endpoint> --adapter openclaw
...
Minimal footprint — just enough for PolyClaw to receive messages from/send to the claw:
- Add a `polyclaw` channel entry (once OpenClaw supports this natively — don't hack it in)
- OR: configure a webhook/hook endpoint that PolyClaw calls
...
### Relation to Vault
During adapter installation, any credentials generated (e.g. a shared secret between PolyClaw and the claw) should be stored in vault, not written to a config file or handed back in plaintext.
...
3. **OpenClaw plugin/channel API** — what's the right hook point for PolyClaw integration? Webhook? Native channel plugin? Check OpenClaw docs/source.
4. **OpenClaw config schema versioning** — is there a machine-readable compatibility manifest? How do we detect version safely?
5. **Approval relay design** — sketch the flow for "agent requests credential → PolyClaw relays approval request to user → user approves in Signal → credential released"

---

## Workstream 3: OpenClaw → PolyClaw/NZC Migration
...
Each channel has exactly one owner at a time: OpenClaw, PolyClaw, or NZC. This is a **decision**, not a copy operation.

Installation question: "Which channels do you want PolyClaw to own?"
- PolyClaw-owned channels: PolyClaw is the router, downstream claws are receivers
- OpenClaw-owned channels: OpenClaw handles them directly, unchanged
- NZC-owned channels: NZC handles them directly

The installer walks through each configured channel and asks who owns it. Credentials for PolyClaw-owned channels get moved to vault. OpenClaw's config for those channels gets disabled (safely, with backup).

### Context Continuity on Channel Reassignment
If a user later reassigns a channel from NZC back to OpenClaw (or vice versa), PolyClaw handles it:
- User requests reassignment ("switch Signal back to OpenClaw")
- PolyClaw updates its routing table
- PolyClaw passes relevant recent history to the newly-assigned claw so it's not starting cold
...
### Config Migration
- Read `openclaw.json`, map known fields to NZC/PolyClaw equivalents
...
polyclaw install
  ...
  → If yes:
      → Channel assignment: for each channel, who owns it? (PolyClaw / OpenClaw / NZC)
      → Memory import: import OpenClaw markdown memory into NZC? [yes/no]
      → Config migration: show diff of mapped settings, confirm
      → Vault setup: store credentials for PolyClaw-owned channels
      → Disable migrated channels in OpenClaw config (with backup)
      → Health check all active claws
```

### Open Questions
- [ ] What's the right data format for PolyClaw's routing table (channel → owner)?
- [ ] How does PolyClaw pass history context on channel reassignment — full dump or summarized?
...
**Option B: PolyClaw meta-commands**
- `!transaction start` / `!transaction commit` / `!transaction rollback` at PolyClaw level
- Human or agent invokes; PolyClaw coordinates with underlying claw's system
...
Session 3 (PolyClaw adapter installation) is the highest-risk session — it edits live OpenClaw config. For that session, use **Backend v1** (backup copy) as the safety mechanism. That's already in the safety requirements. The fuller transaction abstraction can come after.
