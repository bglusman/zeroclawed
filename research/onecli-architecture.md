...
**This is the piece that could be embedded or run alongside NZC/PolyClaw.**
...
### What this means for NZC/PolyClaw integration:
...
3. **Dependency story for NZC**: NZC cannot link OneCLI's gateway as a Rust library dependency. The options are:
   ...
   - **Implement the same proxy pattern natively** in NZC/PolyClaw (build a credential-injecting HTTP proxy from scratch in Rust)
...
## Relevance to PolyClaw's Design

### What PolyClaw Could Borrow Conceptually
...
This pattern can be implemented natively in PolyClaw/NZC without using OneCLI at all.

### What PolyClaw Might Actually Use

For early implementation, running OneCLI as a sidecar alongside PolyClaw is the **fastest path to get credential injection working**. The integration surface is small:
...
### What's Missing from OneCLI for PolyClaw's Use Case
...
OneCLI's blog post explicitly says "approval workflows" (pause + ask human) are "coming next" for high-risk actions — but the implementation will be through their own UI, not via Signal/Telegram. **PolyClaw's chat-based approval relay is genuinely novel and not served by OneCLI.**
...
**Phase 2**: PolyClaw implements its own approval relay on top — when an agent requests a credential that requires human approval, PolyClaw intercepts (either via OneCLI webhook/API or by implementing its own proxy) and sends an approval request to the user via Signal/Telegram. Once approved, credential is released.
...
This is directly applicable to PolyClaw's multi-claw design.
